...
function App() {
  ...
  function paragenrater(){
    if(length > 8){
      alert("please enter Smaller Number than 9")
    }else{
    setGenratedData(para.slice(0,length));
    }
  }
  return (
    <div className="App">
      ...
      <button onClick={paragenrater}
      >GENERATE</button>
      {genratedData.map((elem)=>{
        return(<p className='para'>{elem}</p>)
      })}
      </div>
    </div>
  );
